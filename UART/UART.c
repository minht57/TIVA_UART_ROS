...
int8_t UART_C1101_Read(uint8_t * buf, uint16_t len)
{
    uint16_t idx;
...
int8_t UART_CC1101_Read_CMD(uint8_t * buf)
{
    uint16_t idx;
    if(UART_C1101_QueryData() > 0)
    {
        idx = 0;
        while(C1101_Buf[C1101_ReadIdx++] != ']'){
            if(C1101_Buf[C1101_ReadIdx] == '[')
            {
                idx = 0;
                continue;
            }
